...
void write_entire_comment(FILE* giffy, char* secret_message)
{
  ...
  fseek(giffy, -12 - length - 3, SEEK_CUR); // rewind to length char
  fwrite(&length, sizeof(int), 1, giffy);
  ...
  while (!feof(source)) {
    ...
    if (c != '\0') {
      if (last_c == (signed char)0x21 && c == (signed char)0xFE) {
      	int length = 0;
        fread(&length, sizeof(int), 1, source); // this byte tells us length of message
        
      	message_buffer = (char*)malloc(sizeof(char) * length + 1);
...
void print_directions(void) {
  ...
  printf("$ ./giffy.exe e [input-image-file] [output-filename] [steg-data]\n");
  printf("steg-data is data to embed into image\n");
  printf("\n");
